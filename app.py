# app.py — Spartan AI Demo (Clean & Optimized)
...
with st.sidebar:
    ...
    if st.button("Home"):
        st.session_state.update(messages=[], pending_ocr_text=None, mode="Home")
        st.rerun()
...
if uploaded_file and st.session_state.last_uploaded != uploaded_file.name:
    with st.spinner("Processing image..."):
        try:
            ...
            ocr_text = pytesseract.image_to_string(img, config=OCR_CONFIG).strip() or "(No text found)"
